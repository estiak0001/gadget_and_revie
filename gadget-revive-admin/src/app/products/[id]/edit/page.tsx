...
export default function EditProductPage() {
  ...
  >({});
  // Track the initial values loaded from the product so we can pre-select when attributes arrive
  const initialAttrValuesRef = useRef<ProductAttributeValueEntry[] | null>(null);
  ...
  // Initialise cascading selectors once categories & product are both loaded
  useEffect(() => {
    if (!catInitializedRef.current && categories.length > 0 && formData.category_id) {
      catInitializedRef.current = true;
      const cat = categories.find(c => c.id.toString() === formData.category_id);
      if (cat) {
        if (!cat.parent_id) {
          setCatLevel1(cat.id.toString());
        } else if (!cat.parent?.parent_id) {
          setCatLevel1(cat.parent_id.toString());
          setCatLevel2(cat.id.toString());
        } else {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Stash attribute values for later matching once category attributes have loaded
      if (data.attribute_values && Array.isArray(data.attribute_values)) {
        initialAttrValuesRef.current = data.attribute_values as ProductAttributeValueEntry[];
      }
    } catch (err) {
  ...
  // Load category filter attributes whenever the selected category changes
  useEffect(() => {
    ...
    adminService
      ...
      .then((res) => {
        ...
        setSelectedAttributeValues((prev) => {
          ...
          // Hydrate from initial product attribute values the first time they match
          const initial = initialAttrValuesRef.current;
          for (const a of attrs) {
            if (initial) {
              if (a.input_type === 'text' || a.input_type === 'number') {
                const textMatch = initial.find((pav) => pav.attribute_id === a.id && pav.text_value);
                if (textMatch) {
                  next[a.id] = textMatch.text_value as string;
                  continue;
                }
              } else {
                const matching = initial
                  .filter((pav) => pav.attribute_id === a.id && pav.attribute_value_id)
                  .map((pav) => pav.attribute_value_id as number);
                if (matching.length > 0) {
                  next[a.id] = a.input_type === 'multiselect' ? matching : matching[0];
                  continue;
                }
              }
            }
          ...
          // Clear the ref after the first successful hydration so user edits stick
          if (initial && Object.keys(next).length > 0) {
            initialAttrValuesRef.current = null;
          }
          return next;
        });
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasFiles) {
        ...
        // Append structured attribute values
        formDataToSend.append('attribute_values', JSON.stringify(buildAttributeValuesPayload()));
        ...
        await adminService.updateProduct(productId, formDataToSend);
      } else {
        ...
        payload.attribute_values = buildAttributeValuesPayload();
        ...
        <div className="page-header flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="page-description">{product?.name}</p>
            </div>
          </div>
                  ...
                  {imagePreview && (
                    <div className="mt-3 relative inline-block">
                      ...
                      {imageFile && (
                        <button
                          type="button"
                          onClick={() => { setImageFile(null); setImagePreview(product?.image || ''); }}

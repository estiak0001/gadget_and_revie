...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Restores the top margin the repeated header row otherwise skips on page 2+ — DomPDF
           pins a multi-page table's repeated <thead> to the literal physical top of every
           continuation page, ignoring the page margin entirely (confirmed by testing — even a
           45mm margin didn't move it). Zero border/background so it's genuinely invisible, not
           just short — a blank gap, not a visible empty bar. Also adds the same small gap above
           the header on page 1 — no way to make it page-2-only from pure CSS/HTML. */
        ...
        /* Not done in CSS: DomPDF has no working `counter(pages)` (grand total) — it only
           ...
           InvoiceController@customDownload — DomPDF's actual supported mechanism for
...
<body>
    ...
    {{-- Every position:fixed element that needs to repeat on every page MUST be a direct child of
         ...
         that scan and just renders once, on whichever single page it happened to lay out on. --}}
    ...
    <div class="page-footer-fixed">
        <div class="invoice-footer">
            ...
            <div class="footer-right">
                ...
                <div class="footer-ref-code">{{ $invoice->invoice_number }}</div>
...
<div class="page-bg">
    ...
    <div class="meta-stripe">
        <div class="stripe-cell">
            <span class="lbl">Invoice No:</span> {{ $invoice->invoice_number }}
        </div>
        <div class="stripe-cell center">
            <span class="doc-pill">INVOICE</span>
        </div>
        <div class="stripe-cell right">
            <span class="lbl">Date:</span> {{ $invoice->invoice_date->format('d/m/Y') }}
        </div>
    </div>
    ...
    <div class="body-wrap">
        ...
        <div class="parties-table">
            ...
            <div class="party-cell-right">
                ...
                <div class="party-detail"><strong>Mobile:</strong> {{ $invoice->customer_phone }}</div>
                @endif
                @if($invoice->customer_email)
                <div class="party-detail"><strong>Email:</strong> {{ $invoice->customer_email }}</div>
                @endif
                @if($invoice->customer_address)
                <div class="party-detail"><strong>Address:</strong> {{ $invoice->customer_address }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="items-section">
            ...
            <table class="items">
                <thead>
                    {{-- Blank spacer row — see .thead-spacer's CSS comment above. --}}
                    <tr class="thead-spacer"><td colspan="5"></td></tr>
                    <tr>
                        <th style="width:4%;">#</th>
                        <th style="width:46%;">Item / Description &amp; Serial No.</th>
            ...
            <div class="totals-block">
                <div class="totals-box">
                    <div class="total-row">
                        ...
                        <div class="total-val">BDT {{ number_format($invoice->subtotal, 2) }}</div>
                    </div>
                    @if($invoice->tax > 0)
                    <div class="total-row">
                        <div class="total-lbl">Tax / VAT</div>
                        <div class="total-val">BDT {{ number_format($invoice->tax, 2) }}</div>
                    </div>
                    @endif
                    @if($invoice->shipping > 0)
                    <div class="total-row">
                        <div class="total-lbl">Shipping</div>
                        <div class="total-val">BDT {{ number_format($invoice->shipping, 2) }}</div>
                    </div>
                    @endif
                    @if($invoice->discount > 0)
                    <div class="total-row">
                        <div class="total-lbl">Discount</div>
                        <div class="total-val">- BDT {{ number_format($invoice->discount, 2) }}</div>
                    </div>
                    @endif
                    <div class="grand-total-row">
                        <div class="grand-total-lbl">Total</div>
                        <div class="grand-total-val">BDT {{ number_format($invoice->total, 2) }}</div>
                    </div>
                </div>
            </div>
        </div>
